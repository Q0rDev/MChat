...
public class EntityListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        ...
        EntityHealthChangeEvent dEvent = player.getEntity().getLastDamageCause();
    ...
    @EventHandler
    public void onEntityDamage(EntityHealthChangeEvent event) {
        if (!plugin.mChatEB)
            return;
        ...
        if (event.getSource() instanceof Entity) {
            Entity attacker = (Entity) event.getSource();
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (plugin.healthNotify) {
                ...
                if (messageTimeout) {
                    for (Player players : Spout.getGame().getOnlinePlayers()) {
                        if (players != player) {
                            if (plugin.spoutB) {
                            ...
                            if ((player.getHealth() - event.getChange()) < 1)
                    ...
                    Spout.getGame().getScheduler().scheduleAsyncDelayedTask(plugin, timeRunnable, 20);
                ...
                if (plugin.spoutB) {
                    ...
                    plugin.getGame().getScheduler().scheduleAsyncDelayedTask(plugin, runnable, 4 * 20);
    ...
    void suppressDeathMessage(String pName, String pCause, String world, PlayerDeathEvent event, Integer max, Boolean isPlayer) {
        for (Player player : plugin.getGame().getOnlinePlayers()) {
            ...
            if (!(plugin.getGame().getOnlinePlayers().length > max))
