            ...
            if (args.length > 0) {
                ...
                if (sender instanceof Player) {
                    ...
                    if (plugin.getAPI().checkPermissions(player.getName(), player.getWorld().getName(), "mchat.me"))
                        plugin.getGame().broadcastMessage(plugin.getAPI().ParseMe(player.getName(), world.getName(), message));
                    else
                    ...
                    plugin.getGame().broadcastMessage("* " + senderName + " " + message);
