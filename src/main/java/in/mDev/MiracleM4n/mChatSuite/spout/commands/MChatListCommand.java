...
public class MChatListCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (commandName.equalsIgnoreCase("mchatlist")) {
            ...
            // My Way
            // sender.sendMessage(plugin.getAPI().addColour("&4" + plugin.getLocale().pOffline + ": &8" + plugin.getGame().getOnlinePlayers().length + "/" + plugin.getGame().getMaxPlayers()));

            // Waxdt's Way
            sender.sendMessage(plugin.getAPI().addColour("&6-- There are &8" + plugin.getGame().getOnlinePlayers().length + "&6 out of the maximum of &8" + plugin.getGame().getMaxPlayers() + "&6 Players online. --"));
            formatList(sender);
            return true;
        }
    ...
    void formatList(CommandSender sender) {
        ...
        for (int i = 20; i < plugin.getAPI().addColour("&6-- There are &8" + plugin.getGame().getOnlinePlayers().length + "&6 out of the maximum of &8" + plugin.getGame().getMaxPlayers() + "&6 Players online. --").length(); i++)
            line += "-";
