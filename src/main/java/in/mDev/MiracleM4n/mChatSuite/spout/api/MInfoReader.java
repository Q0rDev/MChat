...
public class MInfoReader {
    ...
    Object getmChatInfo(String name, InfoType type, String world, String info) {
        ...
        if (plugin.mIConfig.getValue(iType + "." + name + ".info." + info) != null)
            return plugin.mIConfig.getValue(iType + "." + name + ".info." + info);

        else if (plugin.mIConfig.getValue(iType + "." + name + ".worlds." + world + "." + info) != null)
            return plugin.mIConfig.getValue(iType + "." + name + ".worlds." + world + "." + info);

        return "";
    }

    Object getmChatGroup(String name) {
        if (plugin.mIConfig.getValue("users." + name + ".group") != null)
            return plugin.mIConfig.getValue("users." + name + ".group");
    ...
    Object getLeveledInfo(String name, String world, String info) {
        ...
        if (plugin.mIConfig.getValue("mchat." + info) == null)
            return "";

        if (plugin.mIConfig.getValue("rank." + info) == null)
            return getBukkitInfo(name, world, info);
        ...
        for (Map.Entry<String, Object> entry : plugin.mIConfig.getValues(true).entrySet()) {
            if (entry.getKey().contains("mchat." + info + "."))
                if (plugin.getAPI().checkPermissions(name, world, entry.getKey())) {
                    ...
                    if (plugin.mIConfig.getValue(rVal) == null)
                        continue;

                    try {
                        iMap.put(plugin.mIConfig.getInteger(rVal), entry.getValue().toString());
    ...
    public String getGroupName(String group) {
        ...
        if (plugin.mIConfig.getValue("groupnames." + group) != null)
    ...
    public String getWorldName(String world) {
        ...
        if (plugin.mIConfig.getString("worldnames." + world) != null)
    ...
    public String getmName(String name) {
        if (plugin.mIConfig.getValue("mname." + name) != null)
