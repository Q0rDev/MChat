    ...
    public void onPlayerChat(PlayerChatEvent event) {
        ...
        if (plugin.mChatPB) {
            ...
            if (plugin.isConv.get(pName)) {
                Player recipient = plugin.getGame().getPlayer(plugin.chatPartner.get(pName));
        ...
        if (plugin.spoutB) {
            ...
            plugin.getGame().getScheduler().scheduleAsyncDelayedTask(plugin, new Runnable() {
                public void run() {
                    SpoutPlayer sPlayer = (SpoutPlayer) plugin.getGame().getPlayer(sPName);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (block.getTypeId() == 63) {
            ...
            if (sign.getLine(0).equals("[mChat]"))
                if (plugin.getGame().getPlayer(sign.getLine(2)) != null)
